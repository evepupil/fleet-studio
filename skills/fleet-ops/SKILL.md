...
- 提示词只写三段：一句话职责、限制、回报格式。「怎么做」的要求（先读什么、怎么验证、要几个来源）交给任务书，写进提示词苦工反而慢：2026-09-24 实测，收集角色删掉「最好两个来源对上」这类要求后，难检索从 3～5 分钟降到 1～2 分钟，结果一样对。
- 限制里都有同样的三条边界：不许再往下派活、不许提交代码、没人会回答它的提问。新写角色照抄这三条。
...
- **有的网关会吞掉系统提示词**，苦工照样干活但不守角色规矩、不按格式回报。接新模型时在系统提示词里放个暗号让它复述，答不上来就别用（`references/pi.md` 记着已知吞提示词的模型）。
- **有的服务只认一部分推理档位**，发了不认的直接 400，苦工第一个请求就失败。接新模型时各档位实打一次，不认的在通道扩展里写档位对照表（`references/pi.md` 推理档位一节）。
