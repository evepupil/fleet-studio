...
| 15 路各自连续发，压 123 秒 | 143 个过 141 个（2 个流中途断开），约 1.2 请求/秒，单个中位 7.4 秒 |
| 写长代码（每个输出 1500 token），5 路、15 路各压 2 分钟左右 | 全过；总产出每秒 261、425 token |

掉的都是网关立刻返回的 500「upstream error: do request failed」（网关连不上朋友的后端）或流中途断开，pi 都会自己重试。首字时间随朋友那边的负载在 1～9 秒之间波动。读上下文约每秒 4000 token（4.8 万 token 第一次要 12 秒），开头相同的再发只要 3 秒，有前缀缓存。

经 fleet 真派只读侦察，三档用同一批问题、同一版提示词：

| 同时跑 | 每条用时 | 整波用时 | 每分钟干完 |
|---|---|---|---|
| 5 路 | 4 分 22 秒～6 分 24 秒 | 5 条 6 分 24 秒 | 0.78 条 |
| 8 路 | 8 分 52 秒～11 分 20 秒 | 8 条 11 分 20 秒 | 0.71 条 |
