...
const REVIEW_RUN1: TimelineDraft[] = [
  ...
  call(
    ...
    "c1",
    "read",
    "packages/core/src/scheduling/dispatchOrder.ts",
  ...
  call(
    ...
    "c2",
    "read",
    "packages/core/src/lifecycle/outcome.ts",
  ...
  thinking(
    13.9,
    "dispatchOrder 在两个项目占用相同时比较队首排队时间，再比较项目键，规则和规格一致。接下来看路径归一化的边角：UNC 路径缺 share 段的情况。",
  ),
  call(
    13.6,
    "c4",
    "read",
    "packages/core/src/projects/path.ts",
  ...
  result(
    ...
    "read",
    true,
    "function resolveDotSegments(segments: string[], rootDepth: number): string[] {\n  const result = segments.slice(0, rootDepth);",
  ),
  call(
    13.2,
    "c5",
    "bash",
    "node -e \"console.log(normalize('\\\\\\\\server\\\\'))\"",
...
const REVIEW_RUN2: TimelineDraft[] = [
  ...
  call(
    ...
    "d2",
    "bash",
    "pnpm exec vitest run packages/core/test/projects",
...
const SCHEMA_RUN: TimelineDraft[] = [
  ...
  result(
    ...
    "read",
    true,
    'export type ParamKind = "number" | "range" | "enum" | "multi_enum" | "bool" | "text";',
  ),
  call(
    2.2,
    "e2",
...
const RETRY_RUN: TimelineDraft[] = [
  call(
    ...
    "f1",
    "read",
    "engine/internal/backend/fake/backend.go",
  ...
  result(
    ...
    "read",
    true,
    "func (b *Backend) Submit(ctx context.Context, req SubmitRequest) (JobID, error) {",
  ),
...
const RELAY_FAIL_RUN: TimelineDraft[] = [
  ...
  ...Array.from({ length: 8 }, (_, index): TimelineDraft[] => {
    ...
    return attempt < 8
      ? [failure(minutes, "Connection error."), retry(minutes, attempt, 2000 * 2 ** (index % 3))]
      : [failure(minutes, "Connection error.")];
  }).flat(),
];
